...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    s = np.sum(np.exp(np.dot(outputVectors, predicted)))
    cost = np.log(s) - np.dot(expected, predicted) #negative log likelihood
    UTvc = np.exp(np.dot(outputVectors, predicted)).reshape(W,1) #exp(U^Tv_c)
    gradPred = np.sum(UTvc/s * outputVectors, axis = 0) - expected
    grad = UTvc/s * predicted
